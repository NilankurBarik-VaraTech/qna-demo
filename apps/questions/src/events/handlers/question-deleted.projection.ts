...
export class QuestionDeletedProjection implements IEventHandler<QuestionDeletedEvent> {
  ...
  async handle(event: QuestionDeletedEvent) {
    console.log(`[QuestionDeletedProjection] Removing question ${event.questionId} from read database`);
    
    await this.questionReadRepo.delete({ id: event.questionId });
    
    console.log(`[QuestionDeletedProjection] Question ${event.questionId} removed from read database`);
  }
}
